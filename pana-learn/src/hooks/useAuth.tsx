...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    const createTestUsers = async () => {
        if (import.meta.env.DEV) {
            ...
            testUsers.forEach((user) => {
                console.log(`${user.email} - Password: ${user.senha}`)
            })
        }
    }
    ...
    const signIn = async (email: string, password: string) => {
        try {
            ...
            if (error) {
                ...
                try {
                    const { data: userData } = await supabase.from("usuarios").select("id").eq("email", email).single()
    ...
    const updateProfile = async (updates: Partial<UserProfile>) => {
        try {
            ...
            const { error } = await supabase.from("usuarios").update(updates).eq("id", userProfile.id)
    ...
    const updateAvatar = async (avatarUrl: string) => {
        try {
            ...
            const { error } = await supabase.from("usuarios").update({ avatar_url: avatarUrl }).eq("id", userProfile.id)
