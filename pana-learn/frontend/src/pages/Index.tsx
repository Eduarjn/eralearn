...
import { useUserProgress } from "@/hooks/useUserProgress" // Fixed import path for useUserProgress hook
import { Badge as BadgeUI } from "@/components/ui/badge"

const Index = () => {
    const { user, loading, userProfile } = useAuth()
    const navigate = useNavigate()
    const { badges, loading: loadingBadges } = useUserBadges(
        userProfile?.id,
    ...
    const { data: progress, loading: loadingProgress } = useUserProgress(
        userProfile?.empresa_id,
        userProfile?.tipo_usuario,
    )
            ...
            <div className="space-y-6">
                <div className="text-center">
                    <div className="page-hero flex flex-col items-center justify-center gap-3 mb-4 py-8">
                        ...
                        <div>
                            <h1 className="text-3xl font-bold text-primary">Bem-vindo ao ERA Learn</h1>
                            <p className="text-contrast text-lg">Sua plataforma de treinamento corporativo</p>
                        </div>
                    </div>
                            ...
                            badges.map((userBadge) => (
                                <div key={userBadge.id} title={userBadge.badge.descricao} className="flex items-center gap-1">
                                    {userBadge.badge.icone_url && (
                                        <img
                                            src={userBadge.badge.icone_url || "/placeholder.svg"}
                                            alt={userBadge.badge.nome}
                                            className="w-6 h-6 rounded-full border border-neutral bg-surface"
                                        />
                                    )}
                                    <BadgeUI className="bg-accent text-primary font-semibold px-3 py-1">{userBadge.badge.nome}</BadgeUI>
                                </div>
                            ))
                        )}
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-surface p-6 rounded-lg">
                        <h3 className="card-title mb-2 text-primary">Treinamentos Disponíveis</h3>
                    ...
                    <div className="bg-surface p-6 rounded-lg">
                        <h3 className="card-title mb-2 text-primary">Progresso Geral</h3>
                        <p className="card-description mb-4 text-contrast">Seu progresso nos treinamentos</p>
                        <div className="text-2xl font-bold text-accent">78%</div>
                    </div>

                    <div className="bg-surface p-6 rounded-lg">
                        <h3 className="card-title mb-2 text-primary">Certificados</h3>
                        <p className="card-description mb-4 text-contrast">Certificados conquistados</p>
